...
const ThemeSapphire = () => {
  ...
  return (
    <div
      ...
      style={{
        // Sapphire & Silver Theme - Elegant & Trustworthy
        ...
        '--border': '210 20% 88%',
        '--ring': '220 80% 45%',
        '--gradient-hero': 'linear-gradient(135deg, hsl(210 40% 96%), hsl(220 30% 92%))',
        '--gradient-primary': 'linear-gradient(135deg, hsl(220 80% 45%), hsl(220 70% 55%))',
        '--shadow-primary': '0 8px 25px -5px hsl(220 80% 45% / 0.35)',
      } as React.CSSProperties}
    >
      <main className="min-h-screen">
        {/* Theme Info Banner */}
        <div className="bg-primary text-primary-foreground py-3 text-center">
          <p className="text-sm font-medium">🎨 Sapphire & Silver - Elegant & Trustworthy Theme | <a href="/" className="underline">View Original</a> | <a href="/theme-royal" className="underline">Royal</a> | <a href="/theme-emerald" className="underline">Emerald</a></p>
        </div>
          ...
          {slides.map((slide, index) => (
            ...
            >
              <div
                ...
                style={{
                  backgroundImage: `linear-gradient(rgba(30,64,175,0.6), rgba(29,78,216,0.7)), url(${slide.image})`
                }}
              />
              <div className="relative container h-full flex items-center justify-center text-center text-white px-4">
                <div className="max-w-4xl">
                  ...
                  <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-bold mb-3 sm:mb-4 bg-gradient-to-r from-slate-200 via-white to-slate-300 bg-clip-text text-transparent leading-tight">
                    {slide.subtitle}
                  </h1>
                  <p className="text-xl sm:text-2xl md:text-3xl lg:text-4xl mb-4 sm:mb-6 md:mb-8">{slide.description}</p>
                  <Button
                    size="lg"
                    className="bg-accent hover:bg-accent/90 text-foreground font-bold px-4 py-4 sm:px-6 sm:py-5 md:px-8 md:py-6 text-sm sm:text-base md:text-lg shadow-lg"
            ...
            {slides.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-colors ${
                  index === currentSlide ? "bg-slate-300" : "bg-white/50"
                }`}
              />
            ))}
          </div>
